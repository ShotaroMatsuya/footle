...
function deleteRow() {
  ...
  $.post('ajax/deleteRow.php', { mode: 'delete' }).done(function (result) {
    if (result != '') {
      if (result == 'success!') {
...
myForm1.addEventListener('submit', () => {
  if (crawlBtn1.disabled === true) {
    ...
    console.log('false');
    crawlBtn1.innerHTML =
      '<span class="spinner-border spinner-border-sm text-light" role="status" aria-hidden="true"></span>Crawling...';
